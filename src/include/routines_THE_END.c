...
void sequence_THE_END()
{
    if(G_INDEX_1 == 2)
    {
        ...
        else if(G_COUNTER_1 == 224)
        {
            ...
            VDP_setTileMapEx(BG_B, image_THE_END.tilemap, TILE_ATTR_FULL(PAL0, TRUE, FALSE, FALSE, G_ADR_VRAM_BG_B), 37, 8, 0, 40, 6, 3, DMA_QUEUE);
        }

        // MEGADRIVE SCORE : RETROVATION //
        else if(G_COUNTER_1 == 358)//358
        {
            VDP_setTileMapEx(BG_B, image_THE_END.tilemap, TILE_ATTR_FULL(PAL0, TRUE, FALSE, FALSE, G_ADR_VRAM_BG_B), 32, 13, 6, 40, 16, 3, DMA_QUEUE);
        }

        // SPECIAL THANKS : 5CUSTOM_GAMES //
        else if(G_COUNTER_1 == 417)//358
        {
            VDP_setTileMapEx(BG_B, image_THE_END.tilemap, TILE_ATTR_FULL(PAL0, TRUE, FALSE, FALSE, G_ADR_VRAM_BG_B), 33, 18, 22, 40, 14, 4, DMA_QUEUE);
        }

        // THE END LOGO //
        else if(G_COUNTER_1 == 490)
        {
            VDP_setTileMapEx(BG_B, image_EMPTY_TILE.tilemap, TILE_ATTR_FULL(PAL0, TRUE, FALSE, FALSE, 0), 32, 8, 0, 0, 17, 13, DMA_QUEUE);
